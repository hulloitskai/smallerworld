...
import { useQueryParams } from "~/helpers/inertia/routing";
...
const RegistrationPage: PageComponent<RegistrationPageProps> = () => {
  ...
  const { pwa_scope: currentScope } = useQueryParams();
  ...
  const { values, getInputProps, submitting, submit, setFieldValue } = useForm({
    ...
    onSuccess: () => {
      let redirectUrl = routes.world.show.path();
      
      // If we're in a PWA context, preserve the scope for the new world
      if (isStandalone && currentScope) {
